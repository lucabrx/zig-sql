...
pub const Cursor = struct {
    ...
    pub fn value(self: *Cursor, schema: *const Schema) !DynamicRow {
        const page = try self.btree.pager.get_page(self.page_num);
        return row.get_leaf_row(page, self.cell_num, schema);
    }
...
test "cursor initialization" {
    ...
    try std.testing.expectEqual(@as(u32, 0), cursor.page_num);
    try std.testing.expectEqual(@as(u32, 0), cursor.cell_num);
